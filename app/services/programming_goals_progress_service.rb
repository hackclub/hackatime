class ProgrammingGoalsProgressService
  ...
  def tracked_seconds_for_goal(goal, now:)
    time_window = time_window_for(goal.period, now: now)
    scope = user.heartbeats_excluding_archived_projects.where(time: time_window.begin.to_i..time_window.end.to_i)
    scope = scope.where(project: goal.projects) if goal.projects.any?

    if goal.languages.any?
      grouped = scope.distinct.pluck(:language).each_with_object(Hash.new { |h, k| h[k] = [] }) do |lang, acc|
        category = lang.presence&.categorize_language
        acc[category] << lang if category.present?
      end
      matching_languages = goal.languages.flat_map { |language| grouped[language] }.compact_blank.uniq
