class EmailVerificationRequest < ApplicationRecord
  ...
  scope :valid, -> { kept.where("expires_at > ?", Time.current) }
  ...
  def verify!
    email_address = user.email_addresses.create!(email: email, source: :signing_in)
    soft_delete!
    email_address
  end
  ...
  def generate_token = self.token ||= SecureRandom.urlsafe_base64(32)
  def set_expiration = self.expires_at ||= 30.minutes.from_now
  def downcase_email = (self.email = email.downcase if email.present?)
end
