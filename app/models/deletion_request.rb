class DeletionRequest < ApplicationRecord
  ...
  def self.create_for_user!(user, reason: nil, reason_details: nil)
    create!(user: user, requested_at: Time.current, status: :pending, reason: reason, reason_details: reason_details)
  end

  def approve!(admin)
    update!(status: :approved, admin_approved_by: admin, admin_approved_at: Time.current,
  ...
  def user_not_banned_from_deletion
    return unless user.present? && user.red?
