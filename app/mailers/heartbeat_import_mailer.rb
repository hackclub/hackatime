class HeartbeatImportMailer < ApplicationMailer
  def import_completed(user, run:, recipient_email:)
    setup_import(user, run)
    mail(to: recipient_email, subject: "Your #{@source_name} import is complete")
  end

  def import_failed(user, run:, recipient_email:)
    setup_import(user, run)
    mail(to: recipient_email, subject: "Your #{@source_name} import failed")
  end
  ...
  def wakatime_manual_download_required(user, recipient_email:)
    ...
    mail(to: recipient_email, subject: "Your WakaTime import needs an export link")
  end

  private

  def setup_import(user, run)
    @user = user
    @run = run
