class HeartbeatImportDumpJob < ApplicationJob
  ...
  retry_on HeartbeatImportDumpClient::TransientError,
    ...
    attempts: 8 do |job, error|
      HeartbeatImportRunner.fail_run_for_error!(import_run_id: job.arguments.first, error:)
    end
  ...
  def request_dump!(run, client)
    run.update!(state: :requesting_dump, started_at: run.started_at || Time.current,
                message: "Requesting data dump...", error_message: nil)

    dump = client.request_dump
    run.update!(
      state: :waiting_for_dump, remote_dump_id: dump[:id],
      remote_dump_status: dump[:status], remote_percent_complete: dump[:percent_complete],
      remote_requested_at: Time.current, message: waiting_message(dump), error_message: nil
    )
    dump
  end
  ...
  def process_dump!(run, dump)
    ...
    elsif !dump[:is_processing] && dump[:download_url].present?
      HeartbeatImportRunner.queue_remote_download(
        run:, download_url: dump[:download_url], remote_dump_status: dump[:status],
        remote_percent_complete: dump[:percent_complete].positive? ? dump[:percent_complete] : 100.0
      )
    else
      run.update!(state: :waiting_for_dump, remote_dump_status: dump[:status],
                  remote_percent_complete: dump[:percent_complete],
                  message: waiting_message(dump), error_message: nil)
  ...
  def poll_timed_out?(run) = (run.remote_requested_at || run.started_at || run.created_at) <= MAX_POLL_DURATION.ago

  def waiting_message(dump)
    status = dump[:status].presence || "Preparing data dump"
    percent = dump[:percent_complete].to_f
    percent.positive? ? "#{status} (#{percent.round}%)" : "#{status}..."
  end

  def fail_run!(run, error)
    HeartbeatImportRunner.fail_run!(
      run, message: HeartbeatImportRunner.failure_message_for(run:, error:),
  ...
  def handle_manual_download_link_required!(run, message)
    ...
    if recipient_email.present?
      HeartbeatImportMailer.wakatime_manual_download_required(run.user, recipient_email:).deliver_later
      HeartbeatImportRunner.fail_run!(
        run, message: "WakaTime needs a recent export download link. Check your email for the next step.",
        notify: false, remote_dump_status: run.remote_dump_status.presence || "failed"
      )
    else
      HeartbeatImportRunner.fail_run!(
        run, message: "#{message} Add an email address to your account, then try again.",
        remote_dump_status: run.remote_dump_status.presence || "failed"
      )
    end
  end
