  ...
  def perform(slack_uid, slack_channel_id, user_name, response_url, enabled)
    ...
    if enabled
      HTTP.post(response_url, json: {
        ...
        text: "<@#{slack_uid}> ran `/sailorslog on` to turn on Hackatime notifications in this channel. Every time they code an hour on a project, a short message celebrating will be posted to this channel. They will also show on `/sailorslog leaderboard`."
      })
    else
      HTTP.post(response_url, json: {
