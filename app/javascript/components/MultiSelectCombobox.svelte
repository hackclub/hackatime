<script lang="ts">
  ...
  // Lazy-mount the bits-ui Popover after the initial page render so it
  // doesn't block Inertia SPA navigations.
    ...
    <Popover.Root bind:open>
      <Popover.Trigger>
        {#snippet child({ props })}
          <div
            {...props}
            class="group min-h-10 cursor-pointer rounded-md border border-surface-200 bg-input px-3 py-2 transition-all duration-200 hover:border-surface-300 focus-within:border-primary/70 focus-within:ring-2 focus-within:ring-primary/35 focus-within:ring-offset-1 focus-within:ring-offset-surface"
          >
            {@render triggerInner()}
          </div>
        {/snippet}
