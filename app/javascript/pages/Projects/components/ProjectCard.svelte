...
<article
  class="relative flex min-h-36 min-w-0 flex-col overflow-hidden rounded-2xl border border-surface-200 bg-dark p-5 transition-colors duration-300 ease-out hover:border-surface-300 {projectHref
    ? 'cursor-pointer'
    : ''}"
>
  {#if projectHref}
  ...
  >
    ...
    {#if project.repository?.last_commit_ago}
      <p class="{chipClass} text-surface-content/50">
        <Clock size={16} />
