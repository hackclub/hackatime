        ...
        <div class="mt-6 grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-6">
          {#each projects_data.projects as project (project.id)}
            ...
            >
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0 flex-1">
                  ...
                  {#if project.repository?.stars}
                    <p class="mt-2 inline-flex items-center gap-1 text-sm text-yellow">
              ...
              <p class="text-2xl font-bold text-primary">{project.duration_label}</p>

              {#if project.repository?.description}
                <p class="line-clamp-2 text-sm leading-relaxed text-surface-content/70">
              ...
              {#if project.broken_name}
                <div class="rounded-lg border border-yellow/30 bg-yellow/10 p-3">
...
<Modal
  ...
  title={pendingStatusAction ? pendingStatusAction.title : "Confirm project change"}
