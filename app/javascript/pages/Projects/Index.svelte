<script lang="ts">
  ...
  const filteredAndSortedProjects = $derived.by(() => {
    ...
    let filtered = projects_data.projects.filter((project) => {
      ...
      {:else}
        ...
        >
          {#each filteredAndSortedProjects as project (project.id)}
            <article
              class={viewMode === "grid"
                ? "flex h-full flex-col gap-4 rounded-xl border border-primary bg-dark p-6 shadow-xs backdrop-blur-sm transition-all duration-300"
                : "flex flex-col gap-4 rounded-xl border border-primary bg-dark p-6 shadow-xs backdrop-blur-sm transition-all duration-300 sm:flex-row sm:items-center sm:justify-between"}
            >
              <div
                class={viewMode === "grid"
              ...
              {#if viewMode === "list"}
                <div class="flex shrink-0 items-center gap-3 text-sm">
                  <p
                    class="text-lg font-bold leading-none text-primary sm:text-xl"
                  >
