<script lang="ts">
  ...
  const editPath = $derived(
    admin_mode
      ? adminOauthApplications.edit.path({ id })
      : doorkeeperApplications.edit.path({ id }),
  );
  const rotateSecretPath = $derived(
    admin_mode
      ? adminOauthApplications.rotateSecret.path({ id })
      : doorkeeperApplications.rotateSecret.path({ id }),
  );
  const backPath = $derived(
    admin_mode
      ? adminOauthApplications.index.path()
  ...
  <div class="grid gap-4 lg:grid-cols-[1fr_270px]">
    <section class="space-y-4">
      {#if admin_mode}
        <article class="rounded-xl border border-surface-200 bg-dark p-5">
          <h2 class="text-lg font-semibold text-surface-content">Owner</h2>
          <div class="mt-4">
            {#if application.owner}
              <AdminUserMention user={application.owner} />
            {:else}
              <p class="text-sm text-yellow">No owner assigned.</p>
            {/if}
          </div>
        </article>
      {/if}
    ...
    <aside class="h-fit rounded-xl border border-surface-200 bg-dark p-4">
      ...
      <div class="mt-3 space-y-2">
        ...
        >
          {admin_mode ? "Back to all applications" : "Back to applications"}
        </Link>
      </div>
    </aside>
  </div>
