...
<section class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-12">
  <Link
    ...
    class="group flex items-start gap-3 p-4 bg-surface border border-surface-200 rounded-lg hover:border-primary transition-colors"
  >
    <div>
...
<section class="mb-10">
  <div class="flex items-baseline justify-between mb-4">
    ...
    <span class="text-sm text-muted"
      >{popular_editors.length} of {all_editors.length}</span
    >
  </div>
    ...
    {#each popular_editors as [name, slug]}
      ...
      >
        <img
          ...
          loading="lazy"
        />
        <span class="text-sm text-surface-content text-center leading-tight"
          >{name}</span
        >
      </Link>
    {/each}
...
<details class="group mb-10">
  ...
  >
    <span class="font-medium text-surface-content"
      >Browse all {all_editors.length} supported editors</span
    >
    <svg
...
<!-- Help footer -->
<aside
  class="p-4 bg-surface border border-surface-200 rounded-lg flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2"
>
