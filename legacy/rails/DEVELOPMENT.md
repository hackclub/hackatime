...
# Set your config
$ cp legacy/rails/.env.example legacy/rails/.env
```

Edit `legacy/rails/.env` to include the following:

```env
# Database configurations - these work with the Docker setup
DATABASE_URL=postgres://postgres:secureorpheus123@legacy-postgres:5432/app_development
SAILORS_LOG_DATABASE_URL=postgres://postgres:secureorpheus123@legacy-postgres:5432/app_development
...
Then, fill out the following fields in `legacy/rails/.env`:
...
```sh
$ docker compose up -d
$ docker compose exec legacy-rails /bin/bash
```
...
```bash
docker compose exec legacy-rails bin/ci
```
